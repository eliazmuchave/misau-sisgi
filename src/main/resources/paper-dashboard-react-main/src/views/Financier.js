            ...
            <Row>
                ...
                <Col md="12">
                    <Card className="card-user">
                        ...
                        <CardHeader>
                            <CardTitle tag="strong">Registar Financiadores</CardTitle>
                        </CardHeader>
                        <CardBody>
...
export async function addFinancierAction({request, params}) {
    const id = params.id;
    let url = "/api/financiers";
    if (id) {
        url = `/api/financiers/${id}`;
    }
    ...
    return redirect("/admin/financiers")
...
export async function agentLoader({params}) {
    const id = params.id;
    const token = getAuthorizationToken();
    const url = `/api/financiers/${id}`;
    const response = await fetch("/api/financiers/"+id, {
        headers: {
